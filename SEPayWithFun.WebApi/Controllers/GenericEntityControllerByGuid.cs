...
namespace SEPayWithFun.WebApi.Controllers
{
    partial class GenericEntityController<TModel, TEntity, TContract>
    {
        ...
        {
            var result = await QuerySet.FirstOrDefaultAsync(e => e.Guid == guid);
        ...
        public virtual async Task<ActionResult<TModel>> PutAsync(Guid guid, [FromBody] TModel model)
        {
            try
            {
                var entity = await QuerySet.FirstOrDefaultAsync(e => e.Guid == guid);
        ...
        public virtual async Task<ActionResult> DeleteAsync(Guid guid)
        {
            try
            {
                ...
                if (entity != null)
                {
                    EntitySet.RemoveByGuid(entity.Guid);
                    await Context.SaveChangesAsync();
                }
